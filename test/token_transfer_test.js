...
contract("Token transfers", async function (accounts) {
    ...
    it("should mint shares for a token that already exists", async function () {
        let amountToMint = 50;
        await nftFractionsRepositoryInstance.mint(erc721MockInstance.address, erc721TokenId, erc1155TokenId, amountToMint, nftOwner);
    ...
    it("should mint shares for a token that does not exists on the chain yet", async function () {
        ...
        await nftFractionsRepositoryInstance.mint(erc721MockInstance.address, newErc721TokenId, newErc1155TokenId, amountToMint, nftOwner);
    ...
    it("should not mint while the contract is paused", async function () {
        ...
        let newErc1155TokenId = 2;
        await truffleAssert.reverts(
            nftFractionsRepositoryInstance.mint(erc721MockInstance.address, newErc721TokenId, newErc1155TokenId, amountToMint, nftOwner),
            "Not allowed while paused");
    });
