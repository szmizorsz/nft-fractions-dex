...
function BuyOrders({ orders, accounts, dexContract, setBuyOrders, web3 }) {
    const classes = useStyles();
    ...
    const rowsDisplay = () => {
        if (orders.length === 0) {
            ...
            return <TableBody>
                {orders.map((row) => (
                    <Row row={row} accounts={accounts} dexContract={dexContract} setBuyOrders={setBuyOrders} web3={web3} />
                ))}
            </TableBody>
        }
    }
